...
\chapter{Wstęp}
    ...
    W nowoczesnym OpenGL potok renderowania składa się z: storzenia buforów na werteksy i indexy, w shaderze werteksów
    przekształcenia położeń werteksów za pomocą macierzy rzutowania i ustalenia koloru dla każdego werteksa,
    przycinanie i rasteryzacja, shader fragmentów wykonuje operacje na pikselach mających na celu 
    ustalenie ostatecznego koloru dla każdego piksela, test głębi, blending \cite{grafika3d} (strona 72). \\

    Macierz rzutowania odpowiada za transformację z układu współrzędnych kamery do układu przycinania 
    (clip coordinate system) \cite{grafika3d} (strona 139) \\

    Istotną optymalizacją było instanced rendering, metoda pozwalająca na renderowanie wielu obiektów na raz 
    przy użyciu jednego draw call. Najpierw należy zainicjalizowawać geometrię obiektu który będzie renderowany, 
    następnie stworzyć tablicę zwierającą infomację między innymi o położeniu każdego z tych obiektów. W ten sposób
    zamiast renderowania każdego obiektu w osobnym draw call, można zrenderować wszystkie obiekty w jednym draw call.

    \section{Wybór technologii}
    Rust jest szybkim kompilowanym językiem programowania, wielokrotnie wybieranym most beloved programming language
    przez społeczność Stack Overflow. Cechuje się \dots

    eGui to biblioteka do renderowania interfejsu graficznego, prosta i kompatybilna z Rust i OpenGL

    Rayon to biblioteka do programowania wielowątkowego w Rust, substytut OpenMP z środowiska C.

    OpenGL jest prosty, odpowiedni do tego typu projektu.

    CUDA jest bardzo wydajnym językiem programowania GPU, wykorzystywanym do obliczeń na GPU firmy Nvidia.

    OpenCL jest bardzo wydajnym językiem programowania wielowątkowego, 
    działa na wielu platfomach i sprzętach różnych producentów.

    \section{Motywacje stojące za wybraniem tego tematu}
    Motywacje osobiste.
    Co jest innowacyjne i użyteczne.

    \section{Język programowania Rust}
    Do programowania wydajnych wielowątkowych symulacji komputerowych 
    ...
    było zbyt wolne. Następnie przy użyciu instanced rendering problem ten został zlikwidowanay po 
    stronie interfejsu graficznego.

    Następnym problemem była wydajność symulacji,

    Wielowątkowość jest trudna do osiągnięcia, wymaga znacznej modyfikacjii algorytmów.

    \section{Wielowątkowość}
    (OpenMP) Wątki charakteryzują się współdzielniem pamięci operacyjnej, co może prowadzić to problemów, takich jak nadpisywanie.
    (MPI )Procesy posiadają swoją odzielną pamięć, dlatego synchronizacja przebiega w inny spoób, taki jak message passing.
    (CUDA) To podejście hybrydowe (bloki, warpy, wątki)  \cite{cuda} (strona 23)
    ...
    $t_{max} = \Delta t \cdot N_{time}$ gdzie $t_{max}$ to czas wykonwania symulacji.

    \begin{equation}
        {\delta_v}^2 = \left( \frac{1}{n} \sum_{i=0}^{n-1} {v_i}^2 \right)  -  \left( \frac{1}{n} \sum_{i=0}^{n-1} v_i \right)^2
    \end{equation}

    Należy liczyć na bierząco $v_{k,j}$ oraz $v_{k,j}^2$ gdzie $k$ to numer cząsteczki a $j = 0, 1, ..., n-1$ to numer chwili czasowej. \\ \\
    ...
    Do rejestru dostajemy się za pomocą dzielienia modulo $j = i \, \%  \, n$, które gwarantuje, że nie wyjdziemy poza tablicę 
    oraz stare wartości będą nadpisywana na bierząco.
    ...
    Twierdzenie o wiriale mówi że energia kinetyczna zmienia się wskutek oddziaływań wewnętcznych i można ją 
    liczyć ze wzoru:
    ...
    Obliczamy pochodną siły jaka odziaływała by na węzeł gdyby został 
    ...
    Pochodna eliminuje wpływ pola grawitacjnego, poniewać jest to wartość stała.
