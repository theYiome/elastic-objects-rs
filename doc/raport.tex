...
\chapter{Wprowadzenie}
    ...
    Do generowania grafiki i animacji został wykorzystany OpenGL, wielojęzyczny i wieloplatformowy 
    ...
    Specyfikacja OpenGL opisuje abstrakcyjne API do rysowania grafiki 2D i 3D. Chociaż możliwe jest 
    zaimplementowanie API w całości programowo, jest ono zaprojektowane do implementacji głównie 
    lub całkowicie sprzętowej.

    API to jest zdefiniowane jako zestaw funkcji, które mogą być wywoływane przez program.
    Chociaż definicje tych funkcji są podobne do tych z języka programowania C, to tak 
    naprawdę są one niezależne od języka od jakiegokolwiek języka. 
    OpenGL jest również wieloplatformowy przez co programy go wykorzystujące mogą 
    działać poprawnie na systemie operacyjnym Windows jak i systemach z rodziny Linux. 
    Specyfikacja nie mówi nic na temat uzyskiwania i zarządzania kontekstem OpenGL, 
    pozostawiając to systemowi okien; OpenGL zajmuje się wyłącznie 
    renderowaniem, nie dostarczając żadnych interfejsów API związanych z wejściem, dźwiękiem lub oknami. \\

    Na cele aplikacji wykorzystane zostało API OpenGL w profilu rdzennym (core profile), 
    ...
    współdzielonej pamięci operacyjnej może prowadzić to problemów, takich jak jej nadpisywanie.
    ...
    polished set of tools to help you along the way'' - \cite{rustbook} (strona 1). Restrykcyjność kompilatora 
...
\chapter{Wstęp teoretyczny}
    ...
    \subsection{Algorytm obliczania temperatury}
    \begin{wrapfigure}{R}{0.45\textwidth}
        \includegraphics[width=0.9\linewidth]{avg.drawio}
    ...
    jest równa średniej z temperatur czterech rzędów najbliższych węzłów. Obliczanie temperatury przy 
    użyciu twierdzenia o wiriale okazało się być bardziej intuicyjne i
    ...
    W celu obliczenia ciśnienia działającego na węzeł, zamykamy go w komorze 
    ...
    Wykorzystując powyżej obliczoną pochodną możemy teraz opisać ciśnienie na ściankach za pomocą wzoru:
    \begin{equation}
    ...
    \end{equation}
    Dodajemy ujemny znak do $p_x$ oraz $d_x$ ponieważ siła jest skierowana do środka komory a 
    ...
    Integracja Verleta to metoda numeryczna stosowana do całkowania równań ruchu Newtona,
    jest często używana do obliczania trajektorii cząstek w symulacjach dynamiki molekularnej i grafice komputerowej. 
    Integrator Verleta zapewnia dobrą stabilność numeryczną, a także inne właściwości, które 
    są ważne w układach fizycznych, takie jak odwracalność w czasie, bez znaczących dodatkowych kosztów 
    obliczeniowych w porównaniu z prostą metodą Eulera.

    Do całkowania przyśpieszenia wybrany został algorytm prędkościowy \emph{Verleta}. Jest on dokładniejszy niż metoda 
    \emph{Eulera} jednocześnie nie będąc od niego wyraźnie wolniejszym obliczeniowo, cechy te dobrze odpowiadają postawionym 
    wymaganiom. Inne metody takie jak algorytm RK4 cechują  się lepszą dokładnością ale za 
...
\chapter{Implementacja}
    ...
    \lstinputlisting[language=C]{code/simulate_multiple.rs}
    ...
    samej komorze co on lub w komorze sąsiadującej z jego komorą. Na rysunku 
    węzły z którymi sprawdzona będzie kolizja z węzłem czarnym oznaczone zostały
    kolorem szarym. Węzły które nie znajdują się w branych pod uwagę komorach 
    ...
    kroku czasowego, krok czasowy będzie przemnażany przez wybraną przez użytkownika wartość większą od $1.0$ za 
    ...
    Program ma formę aplikacji okienkowej. Na ekranie widoczny jest aktualny stan symulacji wraz z panelami ustawień
    zawierającymi interaktywne kontrolki pozwalające zmieniać na bieżąco parametry symulacji i sposób 
    jej wyświetlania.
    ...
    Obrazek pochodzi z systemu operacyjnego Windows ale aplikacja jest również kompatybilna z 
    systemem Linux. Widoczne po lewej stronie panele można ukryć lub ponownie wyświetlić klikając
    przycisk funkcyjny \emph{F1} na klawiaturze.
...
\chapter{Testy}
    ...
    \end{figure}
    Wersja wykonywana na CPU jest nieco szybsza niż ta wykonywana na GPU. Może to wynikać z 
    dużej ilości danych które muszą zostać skopiowane do pamięci karty graficznej.
    ...
    lub znikomo wpływających na wynik obliczeń.
