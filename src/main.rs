...
use std::{collections::HashMap, fs::File, ops::RangeInclusive};
        ...
        current_log_dt += dt * steps_per_frame as f32;
        {
            ...
            if current_log_dt > log_dt {
                let (kinetic, gravity, lennjon, wallrep, objrepu) =
                    calculate_energy_2(&nodes, &connections_map, &objects);
...
fn run_performace_test(object_size: usize, dt: f32, simulation_time: f32) {
    ...
    let mut performance_test_file = std::fs::OpenOptions::new()
        .write(true)
        .append(true)
        .open("data/performance_test.csv")
        .unwrap();
...
fn run_performace_test_optimized(object_size: usize, dt: f32, simulation_time: f32) {
    ...
    println!(
        "{} -> {}",
        object_size,
        get_boundary_nodes(&nodes, spacing * 1.1, 0).len()
    );
    ...
    let mut performance_test_file = std::fs::OpenOptions::new()
        .write(true)
        .append(true)
        .open("data/performance_test_optimized.csv")
        .unwrap();
    let mut csv_performance_writer = csv::Writer::from_writer(performance_test_file);
...
fn cuda(device: &Device) -> Program {
    ...
    let cuda_kernel = include_bytes!("./kernels/add.fatbin");
...
fn opencl(device: &Device) -> Program {
    let opencl_kernel = include_str!("./kernels/add.cl");
